...
router.get('/subscription', authMiddleware, async (req, res) => {
    try {
        const result = await query('SELECT subscription_plan FROM users WHERE email = @email', { email: req.user.email });
        if (!result.recordset.length) return res.status(404).json({ error: 'User not found' });
        res.json({ success: true, subscriptionPlan: result.recordset[0].subscription_plan || 'basic' });
    } catch (e) {
...
router.patch('/subscription', authMiddleware, async (req, res) => {
    try {
        ...
        await query('UPDATE users SET subscription_plan = @plan WHERE email = @email', { plan: planKey, email: req.user.email });

        res.json({
            success: true,
            subscriptionPlan: planKey,
            token: makeToken({ id: req.user.id, email: req.user.email, name: req.user.name }),
        });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
