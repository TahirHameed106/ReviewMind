...
router.post('/register', async (req, res) => {
    try {
        ...
        const user = { user_id: result.recordset[0].user_id, email: email.toLowerCase(), name: name || email.split('@')[0] };
...
router.post('/login', async (req, res) => {
    try {
        ...
        res.json({
            success: true, token: makeToken(user),
...
router.post('/verify-mfa', async (req, res) => {
    try {
        ...
        res.json({ success: true, token: makeToken(user), user: { email: user.email, name: user.name } });
    } catch (e) {
...
router.get('/me', authMiddleware, async (req, res) => {
    try {
        const result = await query('SELECT user_id as id,email,name,mfa_enabled FROM users WHERE email=@e', { e: req.user.email });
